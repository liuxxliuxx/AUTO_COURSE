...
# 1. 替换 _get_video_info：限定在播放器容器内找 video
...
# 3. 替换末尾的 video.play() 兜底：限定容器
old_play_fb = '            self.driver.execute_script("var v=document.querySelector(\'video\'); if(v)v.play();")'
# JS 转义问题，逐字符处理：实际是 self.driver.execute_script("var v=document.querySelector('video'); if(v)v.play();")
old_play_fb = "            self.driver.execute_script("var v=document.querySelector('video'); if(v)v.play();")"
new_play_fb = '''            self.driver.execute_script(
                "var host=document.querySelector('#vjs_videoContStudy')||document;"
                "var v=host.querySelector('video');if(v)v.play();"
            )'''
assert old_play_fb in s, 'FB NOT FOUND'
s = s.replace(old_play_fb, new_play_fb, 1)
