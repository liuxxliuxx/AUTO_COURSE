...
class ZhiHuiShuBot:
    ...
    def get_unfinished_videos(self):
        ...
        for item in all_items:
            try:
                ...
                # Skip if in database
                if self.db.is_completed(title):
                    logger.debug("已完成（数据库）: %s", title)
                    continue
    ...
    def run(self):
        ...
        try:
            ...
            for idx, (title, video_el) in enumerate(unfinished, 1):
                ...
                if success:
                    self.db.mark_completed(title)
                    logger.info("已完成课程: %s", title)
                else:
                    logger.warning("课程超时，标记为完成并继续: %s", title)
                    self.db.mark_completed(title)
    ...
    def _show_completion_report(self):
        """Show a summary of what's been completed."""
        try:
            ...
            logger.info("=" * 50)
            logger.info("已累计完成 %d 个课程视频", count)
            logger.info("=" * 50)
        except Exception:
            pass
